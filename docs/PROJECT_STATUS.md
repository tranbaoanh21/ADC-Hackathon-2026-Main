...
- Day 1: guided landmark discovery stores accepted candidates in PostgreSQL as `AI_DRAFT`; buddy later verifies landmarks on web, creates directed edges by choosing from/to/maneuver and publishes one bounded workplace graph.
- Day 2+: user selects origin and reachable destination with a screen reader; Express computes a deterministic BFS path; camera confirms the origin and later expected landmarks; mobile speaks EN/VI narration generated from the reviewed maneuver.
- `spokenCue` is no longer authored or stored. Express owns deterministic narration; orientation/facing metadata and sensor-derived turns are deferred.
...
- Product API `3.0.0`: structured directed graph contract without authored `spokenCue`; current prototype endpoints retain the `/api/v2` namespace.
...
- A FastAPI AI-service v1.1 vertical slice now exists in `services/ai/` with a
  ...
  expectations after frame-level ground-truth review. Express Product API v3
...
- Bảo Anh owns this application repository's mobile, web, Express, PostgreSQL/Prisma, Product API v3, mock/live AI adapter and end-to-end integration.
...
|---|---|---|---|---|---|
| Bảo Anh | `feat/web-admin-polish` | Accessible EN/VI web/mobile UI plus Product API v3 structured-edge flow and deterministic narration | Implementation complete locally; commit/push in progress | Product API breaking change; AI-service unchanged | Device/screen-reader QA and deployment remain |
...
|---|---|---|---|---|
| Mobile | Bảo Anh | Step-based screen-reader-first Learn/Navigate UI and camera flow implemented; typecheck, 12 state/contrast tests and Android production bundle pass; device/VoiceOver/TalkBack QA pending | Not deployed | TBD |
...
| Express API | Bảo Anh | Product API v3 structured-edge contract on `/api/v2`; graph/BFS/session policy, EN/VI narration, in-memory/Prisma persistence and contract-compatible FastAPI adapter pass locally | Not deployed | TBD |
| PostgreSQL | Bảo Anh | Prisma schema, two migrations, seed and repository verified on clean isolated PostgreSQL; all 48 API/database tests pass | Not provisioned | Secret; never record connection string here |
...
| Product and AI contracts | Product API 3.0.0 and AI service 1.1.0 documented |
